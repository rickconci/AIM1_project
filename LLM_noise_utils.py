...
def GPT_api(system_prompt, prompt, n_responses=1, model="gpt-4o-mini"):
    """
...
def run_LLM(data, system_prompt_final_letter, system_prompt_reasoning, iterations,  model='gpt-4o-mini', options_to_add= options_to_add):
    ...
    for i, question_data in enumerate(data):
        ...
        for _ in range(iterations):
            ...
            extracted_answer = GPT_api(system_prompt_final_letter, LLM_answer[0], 1, model='gpt-4o-mini')
...
def run_LLM_single(data, system_prompt_final_letter, system_prompt_reasoning, iterations,  model='gpt-4o-mini', options_to_add= options_to_add):
    data = copy.deepcopy(data)
    ...
    question_prompt = data['question'] + str(data['options'])
    answer_distribution = []
    ...
    # Save the answer distribution for the question
    answer_distribution_all = answer_distribution
...
def calculate_entropy(answer_distribution_all):
    ...
    if isinstance(answer_distribution_all[0] , list):
        for i, answer_dist in enumerate(answer_distribution_all):
            ...
            if len(counts) == 0:
                normalized_entropy_all[i] = 0
            else:
                normalized_entropy_all[i] = entropy / np.log(len(counts))
            
    else:
        counts = np.array(list(Counter(answer_distribution_all).values()))
        total = np.sum(counts)
        probabilities = counts / total
        probabilities = probabilities[probabilities > 0]  # Avoid log(0)
        entropy = -np.sum(probabilities * np.log(probabilities))
        if len(counts) == 1:
            normalized_entropy_all = 0
        else:
            normalized_entropy_all = entropy / np.log(len(counts))
